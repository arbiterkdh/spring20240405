...
public class Controller22 {
    ...
    @PostMapping("sub4")
    public String sub4(String id, String password, RedirectAttributes rttr) {
    ...
    public String method7(String id, String password, RedirectAttributes rttr) {
        // todo: 아래 코드를 작성하기
        //  id 와 password 가 일치하면, login 성공, 아니면 실패
        //  실패면 login form 으로 이동하라는 redirect 응답
        //  성공이면 로그인 성공했다는 경로로 redirect 응답
        if (id.equals(password)) {
            return "redirect:/main22/sub8";
        }
        rttr.addAttribute("login", "fail");
...
//    }
    ...
    public String method14(String id, String pw, RedirectAttributes rttr) {
        ...
        if (ok) {
            // 로그인 성공시
            // 메인 페이지로 redirect
            rttr.addFlashAttribute("message", "로그인에 성공했습니다.");
